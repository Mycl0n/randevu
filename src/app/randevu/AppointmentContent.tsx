        ...
        <div className="max-w-4xl mx-auto px-4">
          ...
          {step === "select" && (
            <div>
              ...
              <h2 className="text-xl font-semibold mb-4">2. Berber Tercihi (Opsiyonel)</h2>
          ...
          {step === "datetime" && (
            <div>
              <h2 className="text-xl font-semibold mb-4">2. Tarih ve Saat</h2>
              <div className="mb-6">
                <label className="block text-sm font-medium text-dark-700 mb-2">Tarih</label>
                <select className="input-field" value={selectedDate} onChange={(e) => { setSelectedDate(e.target.value); setSelectedTime(""); }}>
              ...
              {selectedServiceData && (
                <div className="card mb-6 bg-primary-50 border-primary-200">
                  <div className="flex justify-between"><span className="font-medium">{selectedServiceData.name}</span><span className="font-bold text-primary-500">{formatPrice(selectedServiceData.price)}</span></div>
                  <div className="text-sm text-dark-500 mt-1">{selectedServiceData.duration} dakika</div>
                </div>
              )}
              <div className="flex justify-between mt-6">
                <button onClick={() => setStep("select")} className="btn-secondary flex items-center gap-2"><FaArrowLeft /> Geri</button>
                {selectedTime && <button onClick={() => setStep("info")} className="btn-primary flex items-center gap-2">Devam Et <FaArrowRight /></button>}
              </div>
            </div>
          )}
          ...
          {step === "info" && (
            <div>
              ...
              <div className="space-y-4 card">
                <div><label className="block text-sm font-medium text-dark-700 mb-2">Ad Soyad *</label><div className="relative"><FaUser className="absolute left-3 top-1/2 -translate-y-1/2 text-dark-400" /><input className="input-field pl-10" value={customerName} onChange={(e) => setCustomerName(e.target.value)} placeholder="Adınız Soyadınız" /></div></div>
                <div><label className="block text-sm font-medium text-dark-700 mb-2">Telefon *</label><div className="relative"><FaPhone className="absolute left-3 top-1/2 -translate-y-1/2 text-dark-400" /><input className="input-field pl-10" value={customerPhone} onChange={(e) => setCustomerPhone(e.target.value)} placeholder="05XX XXX XX XX" /></div></div>
                <div><label className="block text-sm font-medium text-dark-700 mb-2">E-posta</label><div className="relative"><FaEnvelope className="absolute left-3 top-1/2 -translate-y-1/2 text-dark-400" /><input className="input-field pl-10" type="email" value={customerEmail} onChange={(e) => setCustomerEmail(e.target.value)} placeholder="ornek@email.com" /></div></div>
              </div>
              <div className="flex justify-between mt-6">
                <button onClick={() => setStep("datetime")} className="btn-secondary flex items-center gap-2"><FaArrowLeft /> Geri</button>
                <button onClick={handleSubmit} className="btn-primary" disabled={submitting}>{submitting ? "Oluşturuluyor..." : "Randevuyu Onayla"}</button>
              </div>
            </div>
          )}
          ...
          {step === "done" && (
            <div className="text-center py-12">
              <div className="w-20 h-20 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-6"><FaCheck className="text-3xl text-green-500" /></div>
              <h2 className="text-2xl font-bold mb-4">Randevunuz Oluşturuldu!</h2>
              <p className="text-dark-500 mb-8">{selectedDate && formatShortDate(selectedDate)} tarihinde, {selectedTime} saatinde randevunuz alınmıştır.</p>
